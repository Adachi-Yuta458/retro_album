...
export default function Login() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setErr(e?.body?.error === "invalid_credentials" ? "メールかパスワードが違うようです" : "ログインに失敗しました");
    } finally {
      setBusy(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <Pressable disabled={busy} onPress={submit} style={[styles.primaryBtn, busy && { opacity: 0.6 }]}>
          {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryBtnText}>ひらく</Text>}
        </Pressable>

        <Pressable onPress={() => router.push("/(auth)/signup")} style={styles.secondaryBtn}>
          <Text style={styles.secondaryBtnText}>はじめての方はこちら</Text>
        </Pressable>
      </KeyboardAvoidingView>
    </LinearGradient>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 36, color: colors.ink, fontWeight: "700", letterSpacing: 2 },
